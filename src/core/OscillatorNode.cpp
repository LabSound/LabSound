...
namespace lab
{
    ...
    OscillatorNode::OscillatorNode(const float sampleRate)
        : m_sampleRate(sampleRate), m_type(std::make_shared<AudioSetting>("type", "TYPE", s_types)), m_firstRender(true), m_virtualReadIndex(0), m_phaseIncrements(AudioNode::ProcessingSizeInFrames), m_detuneValues(AudioNode::ProcessingSizeInFrames)
    {
