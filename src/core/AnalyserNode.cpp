...
namespace lab
{
    ...
    void AnalyserNode::setMinDecibels(double k) { _detail->_minDecibels->setFloat(static_cast<float>(k)); }
    double AnalyserNode::minDecibels() const { return _detail->m_analyser->minDecibels(); }

    void AnalyserNode::setMaxDecibels(double k) { _detail->_maxDecibels->setFloat(static_cast<float>(k)); }
    double AnalyserNode::maxDecibels() const { return _detail->m_analyser->maxDecibels(); }

    void AnalyserNode::setSmoothingTimeConstant(double k) { _detail->_smoothingTimeConstant->setFloat(static_cast<float>(k)); }
    double AnalyserNode::smoothingTimeConstant() const { return _detail->m_analyser->smoothingTimeConstant(); }

    void AnalyserNode::setFftSize(ContextRenderLock &, size_t sz) { _detail->_fftSize->setUint32(static_cast<uint32_t>(sz)); }
    size_t AnalyserNode::fftSize() const { return _detail->m_analyser->fftSize(); }
