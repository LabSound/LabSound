...
AudioBus::AudioBus(const AudioBus & rh)
: m_length(rh.m_length)
, m_busGain(rh.m_busGain)
, m_isFirstTime(true)
, m_sampleRate(rh.m_sampleRate)
{
    for (uint32_t i = 0; i < rh.numberOfChannels(); ++i)
        m_channels.emplace_back(std::make_unique<AudioChannel>());
    for (uint32_t i = 0; i < rh.numberOfChannels(); ++i)
        m_channels[i]->copyData(rh.m_channels[i].get());
}

AudioBus::AudioBus(size_t numberOfChannels, size_t length)
: m_length(length)
{
    for (uint32_t i = 0; i < numberOfChannels; ++i)
    {
		AudioChannel * newChannel = nullptr;
        newChannel = new AudioChannel();
        newChannel->setSize(length);
...
void AudioBus::setLength(size_t newLength)
{
	...
	for (size_t i = 0; i < m_channels.size(); ++i)
		m_channels[i]->setSize(newLength);
}
...
void AudioBus::discreteCopyFrom(const AudioBus & sourceBus)
{
    ...
    else if (numberOfDestinationChannels > numberOfSourceChannels)
    {
        ...
        for (size_t i = numberOfSourceChannels; i < numberOfDestinationChannels; ++i)
        {
            channel(i)->zero();
        }
    }
...
void AudioBus::copyWithGainFrom(const AudioBus & sourceBus, float * lastMixGain, float targetGain)
{
    ...
    sources.resize(numberOfChannels);
    std::vector<float*> destinations;
    destinations.resize(numberOfChannels);
...
std::unique_ptr<AudioBus> AudioBus::createBySampleRateConverting(const AudioBus* sourceBus, bool mixToMono, float newSampleRate)
{
    ...
    if (sourceSampleRate == destinationSampleRate)
	{
        ...
        // Return exact copy.
        return std::make_unique<AudioBus>(*sourceBus);
    }
    ...
    size_t destinationLength = static_cast<size_t>(sourceLength / sampleRateRatio);

    // Create destination bus with same number of channels.
    size_t numberOfDestinationChannels = resamplerSourceBus->numberOfChannels();
