...
void AudioDestinationRtAudio::render(int numberOfFrames, void * outputBuffer, void * inputBuffer)
{
    ...
    if (_numOutputChannels)
    {
        ...
        {
            delete _renderBus;
            _renderBus = new AudioBus(_numOutputChannels, numberOfFrames);
    ...
    if (_numInputChannels)
    {
        ...
        {
            delete _inputBus;
            _inputBus = new AudioBus(_numInputChannels, numberOfFrames);
    ...
    if (_numInputChannels)
    {
        for (uint32_t i = 0; i < _inputBus->numberOfChannels(); ++i)
        {
    ...
    if (_numOutputChannels)
    {
        ...
        for (uint32_t i = 0; i < _renderBus->numberOfChannels(); ++i)
        {
            AudioChannel * channel = _renderBus->channel(i);
