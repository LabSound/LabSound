...
int main(int, char**)
{
    ...
    vector<shared_ptr<AudioNode>> notes;
    {
        ...
        for (int bar = 0; bar < 10; bar++)
        {
            ...
            notes.emplace_back(kick.play(r, drumGain, time));
            notes.emplace_back(kick.play(r, drumGain, time + 4 * eighthNoteTime));
            
            // Play the snare drum on beats 3, 7
            notes.emplace_back(snare.play(r, drumGain, time + 2 * eighthNoteTime));
            notes.emplace_back(snare.play(r, drumGain, time + 6 * eighthNoteTime));
            
            // Play the hi-hat every eighth note.
            for (int i = 0; i < 8; ++i)
            {
                notes.emplace_back(hihat.play(r, drumGain, time + i * eighthNoteTime));
            }
        }
    }
