...
public:
    ...
    std::shared_ptr<SampledAudioNode> play(ContextRenderLock&, float when = 0.0f);

    // play a sound on a particular node, starting after a certain delay
    std::shared_ptr<SampledAudioNode> play(ContextRenderLock&, std::shared_ptr<AudioNode> outputNode, float when = 0.0f);
    ...
    std::shared_ptr<SampledAudioNode> play(ContextRenderLock&, float start, float end, float when = 0.0f);

    // creates a source node sharing the audio buffer but does not connect it to anything
    std::shared_ptr<SampledAudioNode> create(ContextRenderLock& r, float sampleRate);
};
