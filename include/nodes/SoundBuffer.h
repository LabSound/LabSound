...
public:
	...
	std::shared_ptr<WebCore::AudioBufferSourceNode> play(ContextGraphLock& g, ContextRenderLock&, float when = 0.0f);

    // play a sound on a particular node, starting after a certain delay
    std::shared_ptr<WebCore::AudioBufferSourceNode> play(ContextGraphLock& g, ContextRenderLock&,
                                                         std::shared_ptr<WebCore::AudioNode> outputNode, float when = 0.0f);
    ...
    std::shared_ptr<WebCore::AudioBufferSourceNode> play(ContextGraphLock& g, ContextRenderLock&,
                                                         float start, float end, float when = 0.0f);

    // creates a source node sharing the audio buffer but does not connect it to anything
    std::shared_ptr<WebCore::AudioBufferSourceNode> create(ContextGraphLock& g, ContextRenderLock& r, float sampleRate);
};
